        ...
        private static Func<FuncCondition<StrategyContext>> fractalCondition = () =>
           new FuncFractalCondition(
               ...
               onSuccessAction: ctx => {
                   ...
                   {
                       var fractolContextInfo = new FractolContextInfo(BooleanIndicators.GetFractolIndicator(ctx.Lb, 5, Options.FractolStrength), ctx.Lb.Candles.TakeLast(5).ToList());

                       return ctx.PlaceOrderInlineCandleSentiment(ctx.Lb.LastCandle, Side.ShortSell)
                   ...
                   if (BooleanIndicators.GetFractolIndicator(ctx.Lb, 5, Options.FractolStrength) == Fractol.Bullish)
                   {
                       ...
                       return ctx.PlaceOrderInlineCandleSentiment(ctx.Lb.LastCandle, Side.Buy)
                       .AddContextInfo(fractolContextInfo);
                   }
